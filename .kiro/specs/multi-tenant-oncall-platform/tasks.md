...
- [x] 1. 建立数据模型和多租户隔离基础 ✅
  ...
  - [x] 1.5 🆕 创建数据库连接池和租户上下文管理（`storage/database.py`）
    - ❌ **审核发现**：design.md 定义了 `storage/database.py` 但 tasks 遗漏，RLS 依赖 `SET app.tenant_id` 上下文注入
    - 实现 async SQLAlchemy session factory + 连接池
    - 实现 `set_tenant_context(tenant_id)` — 在每个请求中 `SET LOCAL app.tenant_id = '{tenant_id}'`
    - 实现连接归还时清除 `app.tenant_id`（连接池复用安全）
    - _需求：2.2（RLS 策略依赖此模块生效）_
  
  - [x]* 1.2 编写 Property Test：租户数据完全隔离
    - **Property 2: 租户数据完全隔离**
    - **验证需求：2.2, 2.4, 2.6**
    - 使用 hypothesis 生成随机租户 ID 和测试数据
    - 验证租户 A 查询不会返回租户 B 的数据（PostgreSQL、Milvus、Redis）
  
  - [x]* 1.3 编写 Unit Tests：数据模型验证
    - 测试表创建和约束（外键、唯一性）
    - 测试 RLS 策略生效
    - 测试边缘情况（空值、超长字符串）
...
- [x] 2. 实现 JWT 认证和租户上下文注入 ✅
  ...
  - [x]* 2.2 编写 Property Test：JWT Round-Trip 保持身份信息
    - **Property 1: JWT Round-Trip 保持身份信息**
    - **验证需求：1.3, 1.4**
    - 验证任意有效的 tenant_id 和 user_id 编码后解码保持一致
  
  - [x]* 2.3 编写 Unit Tests：认证错误处理
    - 测试无效 JWT（过期、签名错误、缺少字段）
    - 测试 API Key 不存在或已撤销
    - 测试权限不足场景（403 错误）

- [x] 2.5 🆕 实现 LLM Factory 抽象层（从 Qwen/DashScope 迁移到 OpenRouter）
  - ❌ **审核发现**：ONCALL_FINAL.md 要求 "LLM 抽象层，配置切换"，design.md 缺少此模块，原代码 6 个文件硬编码 `ChatQwen`
  - 定义 `LLMFactory` 接口，支持通过配置切换 LLM 提供商（OpenRouter / OpenAI / Anthropic）
  - 在 `config.py` 中替换 `dashscope_*` 配置为 `openrouter_api_key` + `default_model` + `embedding_model`
  - 使用 `langchain_openai.ChatOpenAI` + OpenRouter base_url 替换所有 `ChatQwen` 调用
  - 替换 `DashScopeEmbeddings` 为 `OpenAIEmbeddings`（或 OpenRouter embedding）
  - 移除 `langchain_qwq` 和 `dashscope` 依赖
...
- [x] 5. 实现四层上下文 Compaction ✅（描述已补充）
  - [x] 5.1 实现上下文组装和 Compaction 逻辑
    ...
    - 使用 LLM（通过 LLM Factory）生成 branch_summary 并存入 Redis
    - 保留最近 3-5 轮完整消息，丢弃更早消息
    - 更新 Redis 压缩计数器
    - _需求：5.1, 5.2, 5.3, 5.4, 5.5_
  
  - [x]* 5.2 编写 Property Test：四层上下文结构完整性
    - **Property 7: 四层上下文结构完整性**
    - **验证需求：5.1**
    - 验证组装的上下文包含所有四层
  
  - [x]* 5.3 编写 Property Test：Compaction 触发和执行
    - **Property 8: Compaction 触发和执行**
    - **验证需求：5.2, 5.3, 5.4, 5.5**
    - 验证超过阈值时触发 Compaction
    - 验证生成 branch_summary 并正确存储
  
  - [x]* 5.4 编写 Unit Tests：Compaction 边缘情况
    - 测试空历史消息
...
- [x] 6. 实现 Token 消耗 A/B 测试 ✅
  ...
  - [x]* 6.2 编写 Property Test：Token 消耗记录完整性
    - **Property 18: Token 消耗记录完整性**
    - **验证需求：11.3**
    - 验证每次 LLM 调用都记录 token 使用
  
  - [x]* 6.3 编写 Unit Tests：基准测试验证
    - 测试模拟场景加载
    - 测试 token 计数准确性
    - 测试报告生成

- [x] 6.5 🆕 实现事故响应时间评估
  - ❌ **审核发现**：原在 P2（Task 22），但这是简历核心数字之一（80%+ reduction），三个简历数字必须在 P0 全部产出
  - 在 evaluation/response_time.py 中实现响应时间评估
  - 记录事故从 created_at 到 resolved_at 的完整时长
  - 维护人工处理基线时间（通过历史数据或专家估算）
  - 计算 AI 处理时间相比基线的减少百分比，目标：>= 80%
  - 在 20 个模拟事故场景中进行测试
  - _需求：12.1, 12.2, 12.3, 12.4_

- [x] 7. Checkpoint：P0 阶段验证
  - [x] 运行所有 P0 阶段的单元测试和属性测试
  - [x] 验证 RAGAS Faithfulness >= 85%
  - [x] 验证 Token 减少率 >= 70%
  - [x] 验证事故响应时间减少率 >= 80%
  - [x] 确保改造后的代码能启动 FastAPI 服务，至少 `/health` 端点可访问
...
- [ ] 13. Checkpoint：P1 阶段验证
  ...
  - 🆕 **最小可运行 Demo**：创建 `/api/incident/trigger` 端点，可用 Postman 触发完整 TRIGGERED → RESOLVED 流程
  - 确认所有测试通过，询问用户是否继续 P2 阶段


### P2 阶段：完整度 - Gateway、RBAC、可观测性、接入

- [ ] 14. 实现 Gateway 层 ✅（描述已补充）
  - [ ] 14.1 创建 FastAPI Gateway 和消息路由
    - 在 gateway/server.py 中创建 FastAPI 应用入口
    - 在 gateway/router.py 中实现消息路由逻辑
    - 🆕 **补充**：实现 `agents/supervisor.py` 或将路由判断逻辑写入 `gateway/router.py`（design.md L162 定义了 Supervisor 模块）
...
- [x] 22. ~~实现事故响应时间评估~~ ❌ **已提前到 P0 阶段 Task 6.5**
  - ❌ **审核原因**：简历核心数字（80%+ reduction）不应放在 P2，已移至 P0
  - Grafana Dashboard 展示 per-tenant 平均处理时长 仍在 P2 Task 21（Prometheus 指标）中覆盖
...
- [ ] 24. 实现开发环境 Mock 支持 ✅（描述已补充）
  - [ ] 24.1 创建开发环境配置
    - 🆕 **补充**：创建 `docker-compose.dev.yml`（开发环境）和 `docker-compose.test.yml`（测试环境，design.md L1547-1575 已定义）
...
- [ ] 31. 🆕 配置 CI/CD Pipeline
  - ❌ **审核发现**：design.md L1459-1481 定义了完整的 GitHub Actions 配置，但 tasks 完全未覆盖
  - 创建 `.github/workflows/test.yml`
  - 配置 CI 服务（PostgreSQL、Redis、Milvus）
  - 运行单元测试 + 属性测试 + 覆盖率检查（>= 80%）
  - 配置 pre-commit hooks（black + isort + ruff + mypy）

- [ ] 32. Checkpoint：P2 阶段验证
...
本实现计划共 **35 个顶层任务**（含审核后新增 3 个），按 P0/P1/P2 三个阶段组织：

- **P0 阶段（10 个任务）**：建立多租户基础 + LLM Factory + 产生全部 3 个可量化指标（RAGAS 85%+、Token 减少 70%+、响应时间减少 80%+）
- **P1 阶段（6 个任务）**：实现架构深度（6 状态机、Checkpoint、YAML Workflow、Session 持久化）+ 最小可运行 Demo
- **P2 阶段（18 个任务）**：完善系统（Gateway、RBAC、可观测性、Slack 接入、完整 API、CI/CD）
- **集成测试（1 个任务）**：端到端验证
...
| 修改项 | 原状态 | 修改内容 | 原因 |
|---|---|---|---|
| Task 1.5 🆕 | 不存在 | 新增 `storage/database.py` | design.md 定义了此模块，RLS 依赖 tenant 上下文注入 |
| Task 2.5 🆕 | 不存在 | 新增 LLM Factory（OpenRouter） | 原代码绑定 Qwen/DashScope，所有后续 Agent 任务依赖此 |
| Task 5.1 | 无 ContextAssembler | 补充 `agents/context.py` | design.md L308-334 定义了接口 |
| Task 6.5 🆕 | 在 P2 (Task 22) | 提前到 P0 | 简历核心数字，必须在 P0 产出 |
| Task 7 | 无可运行验证 | 增加 FastAPI 启动验证 | P0 结束后应能启动服务 |
| Task 13 | 无 Demo | 增加最小可运行 Demo 端点 | P1 结束后应能用 Postman 测试 |
| Task 14.1 | 无 Supervisor 提及 | 补充 `agents/supervisor.py` | design.md L162 定义了此模块 |
| Task 22 | P2 阶段 | 标记已移至 P0 | 简历数字优先级调整 |
| Task 24.1 | 只提 Docker Compose | 补充 dev/test 两套配置 | design.md 有完整测试环境配置 |
| Task 31 🆕 | 不存在 | 新增 CI/CD Pipeline | design.md L1459-1481 定义但未覆盖 |
| 编号调整 | Task 31-32 | 重编号为 32-33 | 为新增 CI/CD 腾出编号 |
